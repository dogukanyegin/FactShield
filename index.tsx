...
// ✅ remove seeded/demo post id(s) like Silent Echo
const REMOVE_IDS = new Set<number>([1]);

function safeParsePosts(raw: string | null): Post[] {
  if (!raw) return [];
  try {
...
const App = () => {
  ...
  // ✅ Load posts/user from localStorage (NO mass delete; only filter unwanted IDs)
  useEffect(() => {
    const loaded = safeParsePosts(localStorage.getItem(POSTS_KEY))
      .filter((p) => p && typeof p.id === "number" && !REMOVE_IDS.has(p.id));

    setPosts(loaded);
    savePostsToStorage(loaded);
  ...
  // ✅ Failsafe: cannot be in "post" view without active id
  useEffect(() => {
    if (view === "post" && activePostId == null) setView("home");
  }, [view, activePostId]);
  ...
  const renderHome = () => (
    <div className="space-y-6">
      <div className="text-osint-muted font-mono text-sm">
        TOTAL POSTS LOADED: <span className="text-white">{posts.length}</span>
      </div>
  ...
  const renderPostDetail = () => {
    ...
    return (
      <div className="bg-osint-card border border-[#333] rounded-lg p-8 shadow-xl">
        ...
        <div className="prose prose-invert max-w-none font-sans whitespace-pre-wrap text-lg leading-relaxed mb-8">{post.content}</div>
      ...
      <div className="bg-osint-card border border-[#333] rounded-lg p-6">
        ...
        <form onSubmit={handleAddPost} className="space-y-4">
          ...
          <div>
            ...
            <textarea name="content" rows={8} className="w-full bg-[#121212] border border-[#333] text-white p-3 rounded focus:outline-none focus:border-osint-green font-sans" placeholder="Enter analysis here..." required></textarea>
          </div>
          <div>
